  ...
  return (
    <div className='m-6 text-center lg:text-start lg:w-[850px] '>
        ...
        <div className='bg-[#37A7AE]  w-[auto] md:w-[300px] md:m-auto lg:m-0  items-center text-center  text-white'>
            ...
            <button  onClick={() => TwoFactor((prev) => !prev)} className="h-[58px] md:w-[auto]   ">Enable Two-factor Authentication</button>
        </div>
        </div>
        ...
        <TwoFactorAuthentification/>
            <div
              className=""
              onClick={() => TwoFactor(false)} // change Changepassword state to false to close 
            >
              <svg
                className="h-8 w-8 text-gray-600"
                viewBox="0 0 24 24"
        ...
        />
        <button onClick={() => LogOut((prev) => !prev)} className='button w-[15rem]'>Log out of all Sessions</button>
        </div>
        <div  className={logOut ? "flex" : "hidden"}  > 
        <LogOutSession/>
            <div
              className=""
              onClick={() => LogOut(false)} // change Changepassword state to false to close 
            >
              <svg
                className="h-8 w-8 text-gray-600"
                viewBox="0 0 24 24"
        ...
        <button onClick={() => DeleteAccount((prev) => !prev)} className=' button w-[11rem]'>Delete Account</button>
        </div>
        <div  className={deleteAccount ?   "flex" : "hidden"}  > 
        <Delete/>
            <div
              className=""
              onClick={() => DeleteAccount(false)} // change Changepassword state to false to close 
            >
              <svg
                className="h-8 w-8 text-gray-600"
                viewBox="0 0 24 24"
