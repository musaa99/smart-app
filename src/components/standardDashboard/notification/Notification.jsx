...
const Notification = ({Notification}) => {
  ...
  return (
      <div className="m-4 ">
      <h1 className="text-[30px] font-bold leading-[45px]  m-auto ">{Notification}</h1>
        ...
        <div className=" ">
          ...
          {reply 
            ? <div> 
                <input
            type="text"
            className=" md:w-[auto] p-4 text-gray-500 border rounded-md outline-none bg-white focus:bg-white focus:border-indigo-600"
          /> 
                <button className=" button bg-slate-400 ml-4 w-[148px]   h-[38px]" onClick={() => SetReply((prev) => !prev)}>send</button> 
              </div> 
            : <button onClick={() => SetReply(true)} className=" button w-[85px]  h-[58px]">Reply</button>
          }
        </div>
      </div>
    </div>
